...
def split_and_convert(args):
    ...
    try:
        config["gpt"]["model_name"] = (
            "opt" if hf_config["_name_or_path"] == "" else hf_config["_name_or_path"]
        )
        ...
        config["gpt"]["layernorm_type"] = (
            "pre_layernorm" if hf_config["do_layer_norm_before"] else "post_layernorm"
        )
        config["gpt"]["activation_type"] = "Relu"
        config["gpt"]["has_post_decoder_layernorm"] = (
            "1" if has_post_decoder_layernorm else "0"
        )
    ...
    for name, param in model_named_parameters_iter:
        if name.find("embed") != -1:
            ...
            model_named_parameters[name] = (
                param.permute(1, 0) if len(param.shape) == 2 else param
            )
    # print(model_named_parameters.keys())
    for l in range(num_layers):
        q_weight = model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.q_proj.weight"
        ]
        k_weight = model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.k_proj.weight"
        ]
        v_weight = model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.v_proj.weight"
        ]
        q_bias = model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.q_proj.bias"
        ]
        k_bias = model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.k_proj.bias"
        ]
        v_bias = model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.v_proj.bias"
        ]
        qkv_weight = fuse_qkv_weight(q_weight, k_weight, v_weight)
        qkv_bias = fuse_qkv_weight(q_bias, k_bias, v_bias)
        model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.qkv_proj.weight"
        ] = qkv_weight
        model_named_parameters[
            f"model.decoder.layers.{l}.self_attn.qkv_proj.bias"
        ] = qkv_bias
    ...
    for name, param in model_named_parameters.items():
        if name == "model.decoder.embed_positions.weight":
            param[padding_offset:, ...].detach().cpu().numpy().astype(
                np_weight_data_type
            ).tofile(saved_dir + "model.wpe.bin")
            ...
            if "model.decoder.project_in.weight" in model_named_parameters.keys():
                ...
                torch.matmul(param, project_in).detach().cpu().numpy().astype(
                    np_weight_data_type
                ).tofile(saved_dir + "model.wte.bin")
                torch.matmul(param, project_out).detach().cpu().numpy().astype(
                    np_weight_data_type
                ).tofile(saved_dir + "model.lm_head.weight.bin")
            else:
